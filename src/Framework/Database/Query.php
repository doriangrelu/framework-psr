...
class Query implements \IteratorAggregate
{
     ...
     * @return array en clef nb nombre d'enregistrement et en clef content un stdclass
     */
    public function query(string $statement, ?array $params = []):array
    {
        ...
        if($request->rowCount()>0){
            $return["nb"]=$request->rowCount();
            $return["content"]=$request->fetchAll(\PDO::FETCH_OBJ);
        }
        return $return;
    }
    ...
    public function update(string $table, array $params, ?array $conditions=[]):bool
    {
        ...
        {
            $prepareArray[]="$key=:$key";
        }
        $prepare=join(",", $prepareArray);
        if(count($conditions)>0){
            $conditionsPrepare=" WHERE ";
            $conditionsArray=[];
            foreach ($conditions as $key=>$value)
            {
                $conditionsArray[]="$key=:$key";
            }
            $conditionsPrepare.=join(" AND ", $conditionsArray);

        }
        $execute=array_merge($params, $conditions);
        $statement= "UPDATE `$table` SET $prepare $conditionsPrepare";
